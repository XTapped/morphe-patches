...
private const val GBOARD_EXTENSION = "extensions/xtapped-gboard.rve"
...
private fun mergeBundledExtension(context: BytecodePatchContext, extension: String) {
    ...
    try {
        ...
        val patchClasses = context.javaClass
            ...
            .invoke(context)
        val classDefClass = loadRuntimeClass(
            runtimeClassLoader,
            "com.android.tools.smali.dexlib2.iface.ClassDef"
        )
        val extensionDex = loadExtensionDex(context, runtimeClassLoader, extensionBytes, extension)
        val extensionClasses = extensionDex.javaClass
            .getMethod("getClasses")
            .invoke(extensionDex) as Iterable<*>
        val classByOrNullMethod = patchClasses.javaClass
            .getMethod("classByOrNull", String::class.java)
        val addClassMethod = patchClasses.javaClass
            .getMethod("addClass\$morphe_patcher", classDefClass)
        val classMergerClass = loadRuntimeClass(
            runtimeClassLoader,
            "app.morphe.patcher.util.ClassMerger"
        )
...
private fun loadExtensionDex(
    ...
    val opcodesClass = loadRuntimeClass(
        runtimeClassLoader,
        "com.android.tools.smali.dexlib2.Opcodes"
    )
